...
//           style={{
//             maxHeight: tasks.length > 0 ? 'calc(100vh - 300px)' : 'auto',
//             scrollbarWidth: 'thin',
...
export function TaskColumn({
  ...
  // Function to get maxHeight based on device
  const getMaxHeight = () => {
    if (tasks.length === 0) return 'auto';

    // If not mobile, use the desktop height
    if (!isMobile) return 'calc(100vh - 300px)';

    // For mobile, use an adaptive approach based on device height
    // For taller devices like iPhone 14 Pro Max and Pixel 7, reduce bottom spacing
    const viewportHeight = window.innerHeight;

    // For smaller screens, minimum UI space of 350px
    if (viewportHeight < 700) {
      // For small screens like iPhone SE
      return 'calc(100vh - 350px)';
    } else {
      // For larger mobile screens like iPhone 14 Pro Max and Pixel 7
      // Less spacing at the bottom (cap at 380px)
      return 'calc(100vh - min(max(320px, 33vh), 380px))';
    }
  };
